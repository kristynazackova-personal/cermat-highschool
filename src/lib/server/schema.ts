/**
 * Schéma databáze — JEDINÝ zdroj pravdy.
 *
 * Je tady v kódu, a ne jen v .sql souboru, ze dvou důvodů: aplikace ho umí
 * sama založit při prvním připojení (viz `ensureSchema` v `db.ts`), takže
 * nasazení nevyžaduje ruční krok, a je zaručeně k dispozici za běhu bez
 * ohledu na to, co sestavení zkopíruje vedle sebe.
 *
 * Každý příkaz je psaný s IF NOT EXISTS a celek běží v transakci, takže
 * opakované spuštění nic nerozbije ani nepřepíše — a dvě instance, které
 * naběhnou zároveň, si nemůžou ublížit.
 *
 * Soubor `db/001_init.sql` se z tohohle řetězce generuje
 * (`npm run db:sql`) a slouží k ručnímu spuštění a ke kontrolám v testech.
 */
export const SCHEMA_SQL = `-- Schéma pro účty a ukládání pokusů.
--
